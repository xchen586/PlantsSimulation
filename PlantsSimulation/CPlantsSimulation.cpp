...
bool CPlantsSimulation::LoadInputHeightMap()
{
	...
	std::vector<std::vector<short>> heightmapNormalBlurShort4096 = MaskedGaussianBlurHeightmap(heightMapShort4096, heightMasksShort4096, 6, 2.0);
	std::vector<std::vector<short>> heightmapIIRBlurShort4096 = iir_gauss_blur_with_mask(heightMapShort4096, heightMasksShort4096, 5);
	...
	char mesh1_heightmap_raw_export[MAX_PATH];
	...
	char short_normal_blur_height_map_export[MAX_PATH];
	char short_iir_blur_height_map_export[MAX_PATH];
	...
	memset(mesh1_heightmap_raw_export, 0, sizeof(char) * MAX_PATH);
	...
	memset(short_normal_blur_height_map_export, 0, sizeof(char) * MAX_PATH);
	memset(short_iir_blur_height_map_export, 0, sizeof(char) * MAX_PATH);
	...
	ExportShortHeightMap(mesh1HeightMapShort4096, mesh1_heightmap_raw_export, 0x00FF0000, true);
	...
	ExportShortHeightMap(heightmapNormalBlurShort4096, short_normal_blur_height_map_export, 0x000000FF, true);
	ExportShortHeightMap(heightmapIIRBlurShort4096, short_iir_blur_height_map_export, 0x000000FF, true);
#else
	...
	ExportShortHeightMapWithMask(heightmapNormalBlurShort4096, heightMasksShort4096, short_normal_blur_height_map_export, 0x0000FF00, false, true);
...
bool CPlantsSimulation::BuildForest()
{
	...
	if (!m_onlyPOIs)
	{
		...
		//m_pForest->generateFast(m_forestAge, m_iteration);
		m_pForest->generateFastAdjustAmount(
			m_forestAge, 
			m_iteration,
			m_gridDelta,
			m_initialDensity,
